defmodule AdventOfCode.Y2025.Day01 do
  ...
  """
  use AdventOfCode.Helpers.InputReader, year: 2025, day: 1
  ...
  <a href="https://en.wikipedia.org/wiki/Project_management"
  target="_blank">project management</a>! This has given them the tools
  they need to prevent their usual Christmas emergency. For example, they
  now know that the North Pole decorations need to be finished soon so
  ...
  *rotations*, one per line, which tell you how to open the safe. A
  ...
  def solve_1(data) do
    {_dial, total} =
      for <<dir::binary-size(1), count::binary>> <- data,
          count = String.to_integer(count),
          reduce: {50, 0} do
        {dial, total} ->
          dial =
            turn_dial(dial, dir, count)
            |> Integer.mod(100)
  ...
  def solve_2(data) do
    {_dial, total} =
      for <<dir::binary-size(1), count::binary>> = _line <- data,
          count = String.to_integer(count),
          reduce: {50, 0} do
        {prev_dial, prev_total} ->
          raw_dial = turn_dial(prev_dial, dir, count)
          new_dial = Integer.mod(raw_dial, 100)

          new_total =
            cond do
              raw_dial > 99 ->
                prev_total + div(raw_dial, 100)

              raw_dial <= 0 and prev_dial > 0 ->
                prev_total + div(raw_dial, -100) + 1
  ...
  # --- </Solution Functions> ---
  defp turn_dial(dial, "L", count), do: dial - count
  defp turn_dial(dial, "R", count), do: dial + count
end
