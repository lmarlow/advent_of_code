defmodule AdventOfCode.Y2025.Day03 do
  ...
  """
  use AdventOfCode.Helpers.InputReader, year: 2025, day: 3
  ...
  could if the escalator weren't also
  ...
  def solve_1(data) do
    ...
    |> Enum.reduce(0, fn [_last | rest] = reversed_digits, acc ->
      tens_digit = Enum.max(rest)

      ones_digit =
        reversed_digits
        |> Enum.reverse()
        |> Enum.drop_while(&(&1 != tens_digit))
        |> Enum.drop(1)
        |> Enum.max()

      tens_digit * 10 + ones_digit + acc
    end)
  end
